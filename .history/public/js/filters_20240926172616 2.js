...
document.addEventListener('DOMContentLoaded', () => {
    ...
    filterButtons.forEach((button) => {
        ...
        button.addEventListener('click', () => {
            ...
            if (button.classList.contains('active')) {
                if (button.parentElement.previousElementSibling.textContent.includes('Category')) {
                    selectedCategories.push(filterValue);
                } else if (button.parentElement.previousElementSibling.textContent.includes('Type of Videos')) {
                    selectedVideoTypes.push(filterValue);
                }
            } else {
                if (button.parentElement.previousElementSibling.textContent.includes('Category')) {
                    const index = selectedCategories.indexOf(filterValue);
                    if (index > -1) selectedCategories.splice(index, 1);
                } else if (button.parentElement.previousElementSibling.textContent.includes('Type of Videos')) {
                    const index = selectedVideoTypes.indexOf(filterValue);
                    if (index > -1) selectedVideoTypes.splice(index, 1);
                }
            }
        });
