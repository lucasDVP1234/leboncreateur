...
app.get('/signup', (req, res) => {
    Creators.find({}, (err, creators) => {
        ...
        // Initialiser des ensembles pour éviter les doublons
        const categoriesSet = new Set();
        const videoTypesSet = new Set();

        // Parcourir chaque créateur pour extraire les catégories et types de vidéos uniques
        creators.forEach(creator => {
            categoriesSet.add(creator.category); // Ajouter la catégorie à l'ensemble

            // Ajouter chaque type de vidéo à l'ensemble
            creator.videoTypes.forEach(type => {
                videoTypesSet.add(type);
            });
        });

        // Convertir les ensembles en tableaux pour les passer à la vue
        const categories = Array.from(categoriesSet);
        const videoTypes = Array.from(videoTypesSet);

        // Rendre la vue en passant les créateurs, les catégories et les types de vidéos uniques
        res.render('creators', { creators, categories, videoTypes });
    });
});
